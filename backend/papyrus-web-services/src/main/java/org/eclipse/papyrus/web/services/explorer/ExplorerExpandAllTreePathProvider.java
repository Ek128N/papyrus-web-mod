    ...
    public IPayload handle(IEditingContext editingContext, Tree tree, ExpandAllTreePathInput input) {
        ...
        if (object.isPresent()) {
            ...
            if (optionalEditingDomain.isPresent()) {
                var optionalResource = optionalEditingDomain.get().getResourceSet().getResources().stream().filter(resource -> treeItemId.equals(resource.getURI().path().substring(1))).findFirst();
