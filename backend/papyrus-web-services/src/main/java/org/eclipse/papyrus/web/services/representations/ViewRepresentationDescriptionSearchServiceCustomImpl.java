...
public class ViewRepresentationDescriptionSearchServiceCustomImpl implements IViewRepresentationDescriptionSearchService {
    ...
    private final Logger logger = LoggerFactory.getLogger(ViewRepresentationDescriptionSearchServiceCustomImpl.class);
    ...
    public ViewRepresentationDescriptionSearchServiceCustomImpl(IDocumentRepository documentRepository, EPackage.Registry ePackageRegistry, IDiagramIdProvider diagramIdProvider, IURLParser urlParser,
    ...
    public Optional<RepresentationDescription> findById(String representationDescriptionId) {
        ...
        if (papyrusDescription.isPresent()) {
            ...
            return this.findRepresentationInRepository(representationDescriptionId);
        }

    }

    private Optional<RepresentationDescription> findRepresentationInRepository(String representationDescriptionId) {
        ...
        if (sourceId.isPresent()) {
            ...
            if (documentEntity.isPresent()) {
                ...
                var searchedView = this.getViewDefinitions(resource).flatMap(view -> view.getDescriptions().stream())
                        .filter(representationDescription -> representationDescriptionId.equals(this.getRepresentationDescriptionId(representationDescription))).findFirst();
    ...
    public Optional<NodeDescription> findViewNodeDescriptionById(String nodeDescriptionId) {
        ...
        if (papyrusDescription.isPresent()) {
            ...
            return this.findNodeInRepository(nodeDescriptionId);
        }

    }

    private Optional<NodeDescription> findNodeInRepository(String nodeDescriptionId) {
        ...
        if (sourceId.isPresent() && sourceElementId.isPresent()) {
            ...
            if (documentEntity.isPresent()) {
                Resource resource = this.loadDocumentAsEMF(documentEntity.get());
                var searchedViewNodes = this.getViewDefinitions(resource).flatMap(view -> view.getDescriptions().stream()).filter(DiagramDescription.class::isInstance)
                        .map(DiagramDescription.class::cast).map(diagramDescription -> this.findNodeDescriptionById(diagramDescription, sourceElementId.get())).flatMap(Optional::stream).findFirst();
    ...
    public Optional<EdgeDescription> findViewEdgeDescriptionById(String edgeDescriptionId) {
        ...
        if (papyrusDescription.isPresent()) {
            ...
            return this.findEdgeInRepository(edgeDescriptionId);
        }

    }

    private Optional<EdgeDescription> findEdgeInRepository(String edgeDescriptionId) {
        ...
        if (sourceId.isPresent() && sourceElementId.isPresent()) {
            ...
            if (documentEntity.isPresent()) {
                Resource resource = this.loadDocumentAsEMF(documentEntity.get());
                var searchedViewEdges = this.getViewDefinitions(resource).flatMap(view -> view.getDescriptions().stream()).filter(DiagramDescription.class::isInstance)
                        .map(DiagramDescription.class::cast).map(diagramDescription -> this.findEdgeDescriptionById(diagramDescription, sourceElementId.get())).flatMap(Optional::stream).findFirst();
