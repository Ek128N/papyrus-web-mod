...
public class EditingContextMetadataProvider implements IEditingContextMetadataProvider {
    ...
    public EditingContextMetadata getMetadata(String editingContextId) {
        var natures = new IDParser().parse(editingContextId).map(this.projectNatureRepository::findAllByProjectId).orElse(List.of()).stream().map(ProjectNatureEntity::getName).map(Nature::new)
                .toList();
