...
==== On Creation
When the Artifact is created in another Artifact, the field nestedArtifact in the parent Artifact must be set.
...
If a Node is created inside a Device or a Node, the field nestedNode must be filled.
...
If a ExecutionEnvironment is created inside a Device, an ExecutionEnvironment or a Node, the field nestedNode must be filled.
...
(the CommunicationPath extends Association)
...
The Deployment link extends Dependency.
==== On Creation
Nothing special.
...
* feature configuration -> NOT USED in Papyrus diagram... -> to confirm with Papyrus TEAM
* source!=target
...
=== Deployment
In a Deployment Diagram, a Deployment can be created between an Artifact and an InstanceSpecification.

==== On Creation
See org.eclipse.papyrus.uml.service.types.helper.DeploymentEditHelper:

 The source element should be added to the UMLPackage.eINSTANCE.getDeployment_DeployedArtifact() feature
 The target element should be added to the UMLPackage.eINSTANCE.getDeployment_Location() feature
 The Deployment element is contained in the target element

==== Can create
See org.eclipse.papyrus.uml.service.types.helper.DeploymentEditHelper:

 Source should be a DeployedArtifact.
 Target should be a DeploymentTarget
...
==== On Reconnect
 Maybe something in org.eclipse.papyrus.uml.service.types.command.DeploymentReorientCommand
...
==== Label
 The edge has the label «deploy». An other label is created after setting the name/label.
