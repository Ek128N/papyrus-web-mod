...
These queries are defined in a similar way mutations are, with backend DTOs and event handlers. Note that queries don't need a _data fetcher_, they are directly handled by the event handlers.
